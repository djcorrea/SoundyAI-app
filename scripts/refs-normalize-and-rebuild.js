...
const loudnessUtils = require('./loudness-utils.js');
const spectralUtils = require('./spectral-utils.js');
...
class GenreProcessor {
  ...
  _calculateGenreAverages() {
    ...
    const avgLufs = this.results.reduce((sum, r) => sum + r.finalMetrics.lufsIntegrated, 0) / n;
    const avgTruePeak = this.results.reduce((sum, r) => sum + r.finalMetrics.truePeakDbtp, 0) / n;
    const avgLRA = this.results.reduce((sum, r) => sum + r.finalMetrics.lra, 0) / n;
    const avgDR = this.results.reduce((sum, r) => sum + r.additionalMetrics.dynamicRange, 0) / n;
    ...
    for (const bandName of bandNames) {
      ...
      avgBands[bandName] = {
        target_db: parseFloat(avgRmsDb.toFixed(1)),
