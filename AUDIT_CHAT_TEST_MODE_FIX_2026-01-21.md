...
**Arquivos Alterados:** `work/api/chat.js`, `work/lib/user/userPlans.js`
...
### Sintomas
- Usuários autenticados no frontend de teste (https://soundyai-teste.vercel.app) recebiam erro "Erro temporário" ao enviar mensagens no chat
...
### 2️⃣ Modificação em `userPlans.js`
...
- Apenas origens específicas de teste são liberadas:
  - `https://soundyai-teste.vercel.app`
  - `https://soundyai-app-soundyai-teste.up.railway.app`
...
- [x] Produção **NÃO** afetada (validação por origem)
...
### Comandos
```bash
git add work/api/chat.js work/lib/user/userPlans.js
git commit -m "🧪 fix: Liberar chat para usuários autenticados no modo teste"
git push origin main
```

### Validação Pós-Deploy
1. Verificar logs do Railway Production durante uso do frontend de teste
2. Confirmar presença de logs `🧪 [USER-PLANS] BYPASS`
3. Testar envio de mensagens no frontend de teste
4. Validar que produção continua funcionando normalmente
...
A correção permite que o **frontend de teste** use o chat sem limites, identificando requisições pela origem, sem afetar o comportamento de produção. A solução é:

- ✅ **Segura:** Apenas origens específicas são liberadas
