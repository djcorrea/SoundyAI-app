...
export async function processAudioComplete(audioBuffer, fileName, options = {}) {
  ...
  try {
    ...
    // ========= 🎯 FASE FINAL: SINCRONIZAÇÃO TABELA ↔ SUGESTÕES =========
    try {
      console.log('[TABLE-SYNC] ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
      console.log('[TABLE-SYNC] 🎯 INICIANDO SINCRONIZAÇÃO DEFINITIVA');
      console.log('[TABLE-SYNC] ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
      
      // Importar módulo de sincronização
      const {
        normalizeMetricKey,
        finalizeSuggestions,
        ensureCompleteness
      } = require('./suggestions-table-sync.js');
      
      // PASSO 1: Construir statusByKey a partir das métricas e targets
      const statusByKey = {};
      const tableData = {};
      
      // Função helper para calcular status (replica lógica do pushRow)
      const calculateStatus = (val, target, tol) => {
        const EPS = 1e-6;
        ...
        if (!Number.isFinite(diff)) return 'warn';
        
        // Determinar status
        const absDiff = Math.abs(diff);
        
        if (tol === 0) {
          // Lógica bandas espectrais
          if (absDiff <= EPS) return 'ok';
          if (absDiff <= 3.0 + EPS) return 'yellow';
          return 'warn';
        } else if (!Number.isFinite(tol) || tol < 0) {
          // Tolerância padrão
          const defaultTol = 1.0;
          if (absDiff <= defaultTol + EPS) return 'ok';
          if (absDiff / defaultTol <= 2 + EPS) return 'yellow';
          return 'warn';
        } else {
          // Lógica padrão
          if (absDiff <= tol + EPS) return 'ok';
          if (absDiff / tol <= 2 + EPS) return 'yellow';
          return 'warn';
        }
      };
      
      // Mapear métricas principais
      const metricsMap = [
        { key: 'lufs', path: 'lufs.integrated', target: customTargets?.lufs?.target, tol: customTargets?.lufs?.tolerance, unit: ' LUFS' },
        { key: 'truepeak', path: 'truePeak.maxDbtp', target: customTargets?.truePeak?.target, tol: customTargets?.truePeak?.tolerance, unit: ' dBTP' },
        { key: 'dynamicrange', path: 'dynamics.dynamicRange', target: customTargets?.dr?.target, tol: customTargets?.dr?.tolerance, unit: ' dB' },
        { key: 'lra', path: 'lufs.lra', target: customTargets?.lra?.target, tol: customTargets?.lra?.tolerance, unit: ' LU' },
        { key: 'stereo', path: 'stereo.correlation', target: customTargets?.stereo?.target, tol: customTargets?.stereo?.tolerance, unit: '' }
      ];
      ...
      for (const m of metricsMap) {
        ...
        if (Number.isFinite(val) && m.target != null) {
          const status = calculateStatus(val, m.target, m.tol);
          statusByKey[m.key] = status;
          tableData[m.key] = { val, target: m.target, tol: m.tol, unit: m.unit, diff: val - (typeof m.target === 'number' ? m.target : (m.target.min + m.target.max) / 2) };
          
          console.log(`[TABLE-SYNC] ${m.key}: val=${val} target=${JSON.stringify(m.target)} status=${status}`);
        }
      }
      
      // Mapear bandas espectrais
      if (finalJSON.spectralBands && customTargets?.bands) {
        for (const bandKey of Object.keys(customTargets.bands)) {
          const bandData = finalJSON.spectralBands[bandKey];
          const targetData = customTargets.bands[bandKey];
          
          if (bandData && targetData && Number.isFinite(bandData.rms_db)) {
            const status = calculateStatus(bandData.rms_db, targetData.target_db, 0); // tol=0 para bandas
            const normalizedKey = normalizeMetricKey(bandKey);
            statusByKey[normalizedKey] = status;
            tableData[normalizedKey] = { 
              val: bandData.rms_db, 
              target: targetData.target_db, 
              tol: 0, 
              unit: ' dB', 
              diff: bandData.rms_db - (typeof targetData.target_db === 'number' ? targetData.target_db : (targetData.target_db.min + targetData.target_db.max) / 2)
            };
            
            console.log(`[TABLE-SYNC] ${normalizedKey}: val=${bandData.rms_db} target=${JSON.stringify(targetData.target_db)} status=${status}`);
          }
        }
      }
      
      console.log('[TABLE-SYNC] statusByKey construído:', Object.keys(statusByKey).length, 'métricas');
      console.log('[TABLE-SYNC] Detalhes:', statusByKey);
      
      // PASSO 2: Filtrar sugestões (remover OK)
      if (Array.isArray(finalJSON.suggestions) && finalJSON.suggestions.length > 0) {
        const beforeCount = finalJSON.suggestions.length;
        finalJSON.suggestions = finalizeSuggestions(finalJSON.suggestions, statusByKey);
        const afterCount = finalJSON.suggestions.length;
        const removedCount = beforeCount - afterCount;
        
        console.log('[TABLE-SYNC] ✅ Sugestões filtradas:', {
          before: beforeCount,
          after: afterCount,
      ...
      if (Array.isArray(finalJSON.aiSuggestions) && finalJSON.aiSuggestions.length > 0) {
        const beforeCount = finalJSON.aiSuggestions.length;
        finalJSON.aiSuggestions = finalizeSuggestions(finalJSON.aiSuggestions, statusByKey);
        const afterCount = finalJSON.aiSuggestions.length;
        const removedCount = beforeCount - afterCount;
        
        console.log('[TABLE-SYNC] ✅ AI Sugestões filtradas:', {
          before: beforeCount,
          after: afterCount,
          removed: removedCount
        });
      ...
      // PASSO 3: Garantir completude (gerar fallbacks)
      if (finalJSON.aiSuggestions) {
        finalJSON.aiSuggestions = ensureCompleteness(finalJSON.aiSuggestions, statusByKey, tableData);
        console.log('[TABLE-SYNC] ✅ Completude verificada:', finalJSON.aiSuggestions.length, 'sugestões finais');
      }
      
      // LOGS OBRIGATÓRIOS FINAIS
      const nonOkKeys = Object.keys(statusByKey).filter(k => statusByKey[k] !== 'ok');
      const okSuggestionsCount = (finalJSON.aiSuggestions || []).filter(s => {
        const key = normalizeMetricKey(s);
        return statusByKey[key] === 'ok';
      }).length;
      
      const missingCount = nonOkKeys.filter(k => {
        const hasSuggestion = (finalJSON.aiSuggestions || []).some(s => normalizeMetricKey(s) === k);
        return !hasSuggestion;
      }).length;
      
      console.log('[TABLE-SYNC] ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
      console.log('[TABLE-SYNC] 📊 RELATÓRIO FINAL');
      console.log('[TABLE-SYNC] ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
      console.log('[TABLE-SYNC] nonOkCount:', nonOkKeys.length);
      console.log('[TABLE-SYNC] suggestionsCount:', (finalJSON.aiSuggestions || []).length);
      console.log('[TABLE-SYNC] okSuggestionsCount:', okSuggestionsCount);
      console.log('[TABLE-SYNC] missingCount:', missingCount);
      console.log('[TABLE-SYNC] ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
      
      // ✅ CRITÉRIOS DE ACEITAÇÃO
      if (okSuggestionsCount === 0 && missingCount === 0) {
        console.log('[TABLE-SYNC] ✅✅✅ SINCRONIZAÇÃO PERFEITA! Nenhuma sugestão para métricas OK, nenhuma métrica não-OK sem sugestão.');
      } else {
        if (okSuggestionsCount > 0) {
          console.warn('[TABLE-SYNC] ⚠️ ATENÇÃO: Ainda existem', okSuggestionsCount, 'sugestões para métricas OK!');
        }
        if (missingCount > 0) {
          console.warn('[TABLE-SYNC] ⚠️ ATENÇÃO: Faltam sugestões para', missingCount, 'métricas não-OK!');
        }
      }
      
    } catch (syncError) {
      console.error('[TABLE-SYNC] ❌ Erro na sincronização (não crítico):', syncError.message);
      console.error('[TABLE-SYNC] Stack:', syncError.stack);
      // Não propagar erro - sincronização é failsafe
    }
