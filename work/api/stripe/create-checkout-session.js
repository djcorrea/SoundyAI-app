...
router.post('/create-checkout-session', async (req, res) => {
  ...
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      mode: 'subscription', // ✅ MODO ASSINATURA (não payment)
      payment_method_types: ['card'],
      line_items: [
        {
          price: planConfig.priceId, // Price ID recorrente
          quantity: 1,
        },
      ],
      // ✅ CUSTOMER: Garantir criação consistente via email
      customer_email: decodedToken.email,
      // ✅ SUBSCRIPTION METADATA: Propagar UID e plan para subscription (crítico para renovações)
      subscription_data: {
        metadata: {
          uid: uid,
          plan: plan,
      ...
      // ✅ SESSION METADATA: Manter também na session (backup)
      metadata: {
        uid: uid,
        plan: plan,
      },
    ...
    console.log(`✅ [STRIPE] Checkout Session (SUBSCRIPTION) criada: ${session.id} | UID: ${uid} | Plano: ${plan}`);

    // 5️⃣ RETORNAR SESSION URL
