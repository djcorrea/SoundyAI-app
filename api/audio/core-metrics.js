...
class CoreMetricsProcessor {
  ...
  async processMetrics(segmentedAudio) {
    ...
    try {
      ...
      const lufsResults = await this.calculateLUFSMetrics(leftChannel, rightChannel, segmentedAudio.sampleRate);
      const truePeakResults = await this.calculateTruePeakMetrics(leftChannel, rightChannel, segmentedAudio.sampleRate);
      const stereoResults = await this.calculateStereoMetrics(leftChannel, rightChannel);
  ...
  ensureOriginalChannels(segmentedAudio) {
    ...
    if (!(leftChannel instanceof Float32Array)) leftChannel = new Float32Array(leftChannel);
    if (!(rightChannel instanceof Float32Array)) rightChannel = new Float32Array(rightChannel);
  ...
  async calculateLUFSMetrics(leftChannel, rightChannel, sampleRate) {
    ...
    try {
      const lufsResults = calculateLoudnessMetrics(leftChannel, rightChannel, sampleRate);
      const integrated = this.sanitizeValue(lufsResults.integrated || lufsResults.lufs_integrated, -70.0);
      const shortTerm = this.sanitizeValue(lufsResults.shortTerm || lufsResults.lufs_short_term, integrated);
      const momentary = this.sanitizeValue(lufsResults.momentary || lufsResults.lufs_momentary, integrated);
  ...
  createSilentLUFSResult() {
    return { integrated: -70.0, shortTerm: -70.0, momentary: -70.0, lra: 0.0 };
  }
  ...
  async calculateTruePeakMetrics(leftChannel, rightChannel, sampleRate) {
    ...
    if (leftMax < 1e-10 && rightMax < 1e-10) return this.createSilentTruePeakResult();
    ...
    const maxLinear = Math.max(leftPeak.true_peak_linear, rightPeak.true_peak_linear);
  ...
  calculateFrequencyBands(averageSpectrum) {
    ...
    const bands = {
      ...
      mid: { min: 250, max: 2000, energy: 0 },
      treble: { min: 2000, max: 20000, energy: 0 }
    };
    for (const [name, band] of Object.entries(bands)) {
      const minBin = Math.floor(band.min / binSize);
      const maxBin = Math.floor(band.max / binSize);
      let energy = 0;
      for (let i = minBin; i <= maxBin; i++) energy += averageSpectrum[i] ** 2;
      band.energy = energy;
    }
    return bands;
  }
  ...
  validateInputFrom5_2(segmentedAudio) {
    ...
    for (const field of required) {
      if (!segmentedAudio.hasOwnProperty(field)) throw new Error(`INVALID_INPUT_5_2: Missing ${field}`);
    }
    if (segmentedAudio.sampleRate !== CORE_METRICS_CONFIG.SAMPLE_RATE) {
      console.warn(`⚠️ SAMPLE_RATE_MISMATCH: Esperado ${CORE_METRICS_CONFIG.SAMPLE_RATE}, recebido ${segmentedAudio.sampleRate}`);
    }
  }
  ...
  calculateStereoWidth(L, R) {
    ...
    for (let i = 0; i < len; i++) {
      const d = L[i] - R[i], s = L[i] + R[i];
      sumDiff += d ** 2; sumSum += s ** 2;
    }
    return sumSum > 0 ? Math.sqrt(sumDiff / sumSum) : 0.0;
  }
  calculateStereoBalance(L, R) {
    const lRMS = this.calculateRMS(L), rRMS = this.calculateRMS(R);
