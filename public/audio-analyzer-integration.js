...
// 🎯 NOVA FUNÇÃO: Renderizar com caps aplicados do referenceComparison
function renderWithCappedComparison(analysis, container) {
    const comparisons = analysis.referenceComparison;
    const titleText = window.PROD_AI_REF_GENRE || "Comparação por Gênero";
    
    console.log('🎯 [CAPS] Renderizando com caps aplicados:', comparisons.length, 'itens');
    
    // Mapear comparisons para formato de tabela
    const rows = [];
    ...
    comparisons.forEach(item => {
        ...
        // 🎯 USAR DELTA_SHOWN (com caps) em vez de calcular manualmente
        let deltaDisplay = '—';
        let cssClass = 'na';
        let deltaValue = null;
        
        if (item.category === 'spectral_bands' && typeof item.delta_shown === 'number') {
            // Para bandas espectrais: usar delta_shown com cap de ±6dB
            deltaValue = item.delta_shown;
            deltaDisplay = `${deltaValue >= 0 ? '+' : ''}${nf(deltaValue)}${unit}`;
            
            // Determinar classe CSS baseada no delta_shown
            ...
            // 🎯 ADICIONAR NOTA EDUCATIVA se existir
            if (item.note && item.delta_capped) {
                deltaDisplay += ` <span style="font-size:10px;opacity:0.8;display:block;margin-top:2px;">${item.note}</span>`;
            }
            
        } else if (Number.isFinite(value) && Number.isFinite(ideal)) {
            // Para outras métricas: calcular delta normal
            deltaValue = value - ideal;
            deltaDisplay = `${deltaValue >= 0 ? '+' : ''}${nf(deltaValue)}${unit}`;
        ...
        rows.push(`<tr>
            <td>${metric}</td>
            <td>${Number.isFinite(value) ? nf(value) + unit : '—'}</td>
            <td>${Number.isFinite(ideal) ? nf(ideal) + unit : 'N/A'}</td>
            <td class="${cssClass}">${deltaDisplay}</td>
        </tr>`);
    });
    
    // Renderizar tabela
    container.innerHTML = `
    <div class="reference-comparison-card">
        <div class="card-title">📌 Comparação de Referência (${titleText}) <span style="color:#52f7ad;font-size:11px;">⚡ COM CAPS DE SEGURANÇA</span></div>
    ...
    console.log('🎯 [CAPS] Renderização concluída com valores capados para segurança');
}
...
function renderReferenceComparisons(analysis) {
    ...
    // 🎯 NOVO: Priorizar referenceComparison com caps aplicados
    if (analysis.referenceComparison && Array.isArray(analysis.referenceComparison)) {
        console.log('🎯 [CAPS] Usando referenceComparison com caps aplicados');
        renderWithCappedComparison(analysis, container);
        return;
    }
