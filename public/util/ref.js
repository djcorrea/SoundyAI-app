 ...
 * 
 * @param {Object} refProfile - Perfil de referência do gênero
 * @param {string} metricKey - Chave da métrica (ex: 'lufs', 'true_peak', 'dr')
 * @returns {{ target: number|null, tol: number|null }} Target e tolerância
 */
export function getMetricTargetTol(refProfile, metricKey) {
 ...
 * 
 * @param {Object} refProfile - Perfil de referência
 * @param {string} bandKey - Chave da banda (ex: 'sub', 'bass', 'mid')
 * @returns {{ target: number|null, tol: number|null }} Target e tolerância
 */
export function getBandTargetTol(refProfile, bandKey) {
    const result = {
        target: null,
        tol: null
    };
 ...
 * @param {any} refProfile - Perfil a validar
 * @returns {boolean} True se válido
 */
export function isValidReferenceProfile(refProfile) {
    if (!refProfile || typeof refProfile !== 'object') {
        return false;
    }
