...
function scoreTruePeak(truePeak, target, min, max, criticalThreshold = 0.0) {
  ...
  // Gate secundário: TP na zona de risco (acima do target, mas ainda válido)
  // V3.3 CORREÇÃO: threshold dinâmico baseado no target do modo
  // - streaming: target=-1.0, max=-1.0 → warning se TP > -0.7 (impossível, já é CLIPPING)
  // - pista: target=-0.3, max=0.0 → warning se TP > 0.0 (impossível, já é CRITICAL)
  // - reference: target=-1.0, max=0.0 → warning se TP > -0.7 AND TP <= 0.0
...
// ============================================================================
// EXPORTAÇÕES - SCRIPT CLÁSSICO (UMD/IIFE)
// ============================================================================
// NOTA: NÃO usar export/import para compatibilidade com carregamento clássico
...
if (typeof window !== 'undefined') {
  ...
  // Teste de sanidade
  window.testScoreEngineV3 = async function() {
    console.group('🧪 Teste Score Engine V3');
    
    // Caso A: TP positivo = deve ficar MUITO BAIXO
    const caseA = await computeScoreV3({
      truePeakDbtp: 2.0,
      lufsIntegrated: -14.0,
      clippingPct: 0,
      dr: 8,
  ...
  // Inicializar versão do localStorage
  const savedVersion = localStorage.getItem('scoreEngineVersion');
  if (savedVersion) {
