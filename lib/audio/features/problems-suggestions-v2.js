...
export class ProblemsAndSuggestionsAnalyzerV2 {
  ...
  analyzeSpectralBands(metrics, suggestions, problems) {
    ...
    // 🆕 Presença (10k-20kHz) - Air e espacialidade
    // 🔧 CORRIGIDO: Range correto 10k-20kHz (não 3-6kHz)
    value = bands.presenca_energy_db ?? bands.presenca?.energy_db ?? bands.presenca ?? bands.presence;
    if (Number.isFinite(value)) {
      this.analyzeBand('presenca', value, 'Presença (10k-20kHz)', suggestions);
    }

    // 🆕 Brilho/Air (4k-10kHz) - Clareza e definição
    // 🔧 CORRIGIDO: Range correto 4k-10kHz (não 6-20kHz)
    value = bands.brilho_energy_db ?? bands.brilho?.energy_db ?? bands.brilho ?? bands.air;
    if (Number.isFinite(value)) {
      this.analyzeBand('brilho', value, 'Brilho (4k-10kHz)', suggestions);
    }
