...
function _computeMixScoreInternal(technicalData = {}, reference = null, force = { AUDIT_MODE:false, SCORING_V2:false, AUTO_V2:true }) {
  ...
  function addMetric(category, key, value, target, tol, opts = {}) {
    ...
    if (isFrequencyBand && typeof window !== 'undefined' && window.__BASELINE_BAND_ENERGIES) {
      ...
      if (baseline && Number.isFinite(baseline.rms_db)) {
        ...
        const baselineDiff = Math.abs(baseline.rms_db - target);
        const improvement = baselineDiff - currentDiff; // positivo = melhorou
        
        if (improvement > 0) {
          // Score B baseado na proporção de melhoria vs tolerância
          const improvementRatio = improvement / tol;
          const scoreB = Math.min(1.0, improvementRatio * 0.8); // máx 80% por melhoria
          ...
          if (window.DEBUG_HYBRID_SCORE) {
            console.log(`[HÍBRIDO] ${key}: A=${(scoreA*100).toFixed(1)}% B=${(scoreB*100).toFixed(1)}% final=${(finalScore*100).toFixed(1)}% improvement=${improvement.toFixed(1)}dB`);
          }
        }
      }
    }
  ...
  try {
    if (typeof window !== 'undefined') {
      ...
      // 🎯 2025-01: SISTEMA HÍBRIDO - Armazenar baseline na primeira análise
      if (!window.__BASELINE_BAND_ENERGIES && metrics.bandEnergies) {
        ...
        console.log('🎯 BASELINE armazenado para sistema híbrido Pista A + Pista B:', Object.keys(window.__BASELINE_BAND_ENERGIES.data));
      }
...
try {
  if (typeof window !== 'undefined') {
    ...
    window.resetBaseline = () => {
      ...
      console.log('🎯 Baseline resetado - próxima análise será nova baseline');
    };
