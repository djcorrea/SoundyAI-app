...
function _computeMixScoreInternal(technicalData = {}, reference = null, force = { AUDIT_MODE:false, SCORING_V2:false, AUTO_V2:true }) {
  ...
  // 🔹 [SoundyAI] Novo cálculo baseado em sugestões
  try {
    // Buscar sugestões em múltiplas fontes
    let suggestions = technicalData?.suggestions || [];
    
    // Se não encontrou no technicalData, tentar fontes globais
    if (suggestions.length === 0) {
      suggestions = window?.lastSuggestions || 
                   window?.currentModalAnalysis?.suggestions || 
                   window?.__CURRENT_ANALYSIS__?.suggestions || 
                   [];
    }
    
    if (typeof window !== 'undefined' && window.USE_SUGGESTION_SCORING && suggestions.length > 0) {

      const bandEnergies = technicalData.bandEnergies || {};
      const maxDelta = 6.0; // Limite máximo considerado (6dB)

      const validBands = suggestions.filter(s =>
        s.type === 'band_adjust' &&
        s.technical?.delta &&
        bandEnergies[s.subtype || s.band]
      );

      if (validBands.length > 0) {
        const scores = validBands.map(s => {
          const delta = Math.abs(s.technical.delta);
          const ratio = Math.min(delta / maxDelta, 1);
          return Math.max(20, 100 - (ratio * 80)); // quanto menor o delta, maior o score
        });

        const avgScore = scores.reduce((a, b) => a + b, 0) / scores.length;

        // Substitui o score principal atual
        result.scorePct = Math.round(avgScore);
        result.method = 'suggestion_based';
        result.classification = classify(result.scorePct);
        console.log(`[SoundyAI] 🎚️ Score baseado em sugestões aplicado: ${result.scorePct}% (${validBands.length} bandas processadas)`);
        console.log(`[SoundyAI] 📊 Deltas processados:`, validBands.map(s => `${s.subtype || s.band}: ${s.technical.delta}dB`));
      } else {
        console.log(`[SoundyAI] ⚠️ Nenhuma banda válida encontrada. Total sugestões: ${suggestions.length}`);
      }
    }
  } catch (err) {
    console.warn('[SoundyAI] ⚠️ Falha no cálculo baseado em sugestões:', err);
  }
