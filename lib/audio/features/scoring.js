...
function scoreTolerance(metricValue, target, tol, invert = false, tolMin = null, tolMax = null, isBandMetric = false) {
  ...
  if (adiff <= sideTol) {
    if (PROGRESSIVE_BAND_SCORE && isBandMetric) {
      // Score progressivo: quanto mais próximo do alvo, maior o score
      const progress = 1 - (adiff / sideTol);  // 0 na borda → 1 no alvo
      const curved = Math.pow(progress, 1.2);   // Curva suave (opcional)
      return curved; // 0..1 range preservado
    } else {
