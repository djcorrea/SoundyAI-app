...
export async function sendWelcomeProEmail({ 
  ...
  try {
    ...
    const sanitizedTags = [
      { name: 'source', value: sanitizeResendTag('hotmart') },
      { name: 'plan', value: sanitizeResendTag('pro') },
      { name: 'transaction', value: sanitizeResendTag(transactionId, 'no-transaction') }
    ];

    console.log(`🏷️ [EMAIL] Tags sanitizadas:`, sanitizedTags);
