...
/**
 * FROM_EMAIL com fallback automático para domínio verificado
 * 
 * ⚠️ IMPORTANTE: 
 * - process.env.FROM_EMAIL_VERIFIED deve conter domínio verificado no Resend
 * - Se não configurado, usa onboarding@resend.dev (sempre funciona)
 * - NUNCA usar domínios não verificados (ex: @soundyai.com.br sem configurar DNS)
 */
const FROM_EMAIL = process.env.FROM_EMAIL_VERIFIED || 'SoundyAI <onboarding@resend.dev>';
...
export async function sendWelcomeProEmail({ 
  ...
  // Validar API Key
  if (!RESEND_API_KEY) {
    console.error('❌ [EMAIL ERROR] RESEND_API_KEY não configurado no ambiente');
    return { 
      success: false, 
